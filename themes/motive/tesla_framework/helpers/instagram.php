	...
	function tt_instagram_data( $username, $cache_hours, $nr_images ) {
	    ...
	    if ( !$instaData ) {
	        ...
	        $json           = wp_remote_retrieve_body(
	            wp_remote_get( $user_profile . "/?__a=1", array( 'sslverify' => false, 'timeout'=> 60 ) )
	            );
	        if ( $json ) {                
	            ( $arr = json_decode( $json, true ) ) && json_last_error() == JSON_ERROR_NONE;
       	       
	            if ( !empty($arr['user']['media']['nodes']) && is_array( $arr['user']['media']['nodes'] ) ) {

	                foreach( $arr['user']['media']['nodes'] as $nr => $item ) {
	                    if( $nr >= $nr_images ) 
	                        break;
	                    array_push( $instaData, array(
	                        ...
	                        'caption'       => !empty($item['caption']) && !empty($item['caption']) ? $item['caption'] : '',
	                        'image'         => $item['display_src'],
	                        'thumb'         => $item['thumbnail_src'],
	                        'link'          => $item['display_src'],
